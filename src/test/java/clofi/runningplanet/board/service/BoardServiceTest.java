...
class BoardServiceTest {
	...
	void createBoardTest(){
		...
		//when
		BoardResponse boardResponse = boardService.create(crewId, createBoardRequest, imageFile);
		Board board = boardRepository.findById(boardResponse.boardId())
			.orElseThrow(() ->new IllegalArgumentException("게시판이 없습니다"));
		List<BoardImage> boardImage = boardImageRepository.findAllByBoard(board);
		//then
		assertThat(board.getId()).isEqualTo(boardResponse.boardId());
